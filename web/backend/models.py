...
class EmotionalState(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    stress_score = db.Column(db.Float)  # 0 to 100
    focus_level = db.Column(db.Float)   # 0 to 100
    detected_emotion = db.Column(db.String(50))  # e.g., 'flow', 'anxious', 'bored', 'frustrated'
    context_tag = db.Column(db.String(100)) # e.g., 'resume_builder', 'mock_interview'
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)

    user = db.relationship('User', backref=db.backref('emotional_history', lazy='dynamic', cascade='all, delete-orphan'))


class SkillFitAssessment(db.Model):
    __tablename__ = 'skillfit_assessments'
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    ...
    fitment_category = db.Column(db.String(100))  # Job-ready, Requires Training, etc.
    fraud_flags = db.Column(db.JSON)  # JSON list of flags
    ...
    user = db.relationship('User', backref=db.backref('skillfit_assessments', lazy=True, cascade='all, delete-orphan'))
    job = db.relationship('JobPosting', backref=db.backref('skillfit_assessments', lazy=True))
