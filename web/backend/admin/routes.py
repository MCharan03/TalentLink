...
from ..models import JobPosting, JobApplication, User, UserData, MockTest, MockInterview, Notification, SystemSetting, Quest, Organization, RecruiterProfile, SkillFitAssessment
from ..utils.ai_utils import generate_job_description, analyze_market_trends
from ..decorators import admin_required
import threading

@admin.route('/skillfit_dashboard')
@admin_required
def skillfit_dashboard():
    assessments = SkillFitAssessment.query.order_by(SkillFitAssessment.created_at.desc()).all()
    
    # Simple aggregation for dashboard stats
    stats = {
        'total': len(assessments),
        'job_ready': len([a for a in assessments if a.fitment_category == 'Job-ready']),
        'requires_training': len([a for a in assessments if a.fitment_category == 'Requires Training']),
        'flagged': len([a for a in assessments if a.fraud_flags])
    }
    
    return render_template('admin/skillfit_dashboard.html', assessments=assessments, stats=stats)
...
@admin.route('/skillfit_heatmap')
@admin_required
def skillfit_heatmap():
    # In a real app, this would be aggregated from EmotionalState and application data
    # Mock data for Karnataka Districts
    districts_readiness = [
        {"name": "Bangalore Urban", "lat": 12.9716, "lng": 77.5946, "readiness": 85, "candidates": 1200},
        {"name": "Mysore", "lat": 12.2958, "lng": 76.6394, "readiness": 72, "candidates": 450},
        {"name": "Hubli-Dharwad", "lat": 15.3647, "lng": 75.1240, "readiness": 65, "candidates": 320},
        {"name": "Belgaum", "lat": 15.8497, "lng": 74.4977, "readiness": 58, "candidates": 280},
        {"name": "Mangalore", "lat": 12.9141, "lng": 74.8560, "readiness": 78, "candidates": 410},
        {"name": "Gulbarga", "lat": 17.3297, "lng": 76.8343, "readiness": 42, "candidates": 150},
        {"name": "Shimoga", "lat": 13.9299, "lng": 75.5681, "readiness": 61, "candidates": 210}
    ]
    return render_template('admin/skillfit_heatmap.html', districts=districts_readiness)
