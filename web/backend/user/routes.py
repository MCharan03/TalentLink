# Aura User Routes - Sentient Career Platform
import os
import json
...
from ..models import UserData, MockTest, JobPosting, JobApplication, Notification, MockInterview, User, ExternalApplication, UserXP, CareerForecast, EmotionalState, SkillFitAssessment

@user.route('/assessment_report/<int:assessment_id>')
@login_required
def assessment_report(assessment_id):
    assessment = SkillFitAssessment.query.get_or_404(assessment_id)
    if assessment.user_id != current_user.id and current_user.role != 'admin':
        abort(403)
    return render_template('user/assessment_report.html', assessment=assessment)

@user.route('/api/telemetry', methods=['POST'])
@login_required
def api_telemetry():
    ...
    context_tag = data.get('context', 'dashboard')
    
    # Simple logic for emotion detection
    emotion = 'flow'
    if stress_score > 70:
        emotion = 'frustrated'
...
@csrf.exempt
def api_dashboard():
    # Reuse dashboard logic but return JSON
    analyses = UserData.query.filter_by(user_id=current_user.id)\
    ...
    skillfit_assessments = SkillFitAssessment.query.filter_by(user_id=current_user.id).all()
...
def dashboard():
    ...
    for a in analyses:
         ...
         resume_scores.append({
             'score': score,
             'timestamp': a.uploaded_at.strftime('%Y-%m-%d')
         })
    ...
    # Trigger Sentient Agency (Asynchronous Agency)
    try:
        from ..utils.agency import SentientAgency
        agency = SentientAgency(current_user.id)
        agency.wake_up()
    except Exception as e:
        current_app.logger.error(f"Sentient Agency Error: {e}")
...
@login_required
def interview_prep():
    # Fetch user's mock interview history
    interviews = MockInterview.query.filter_by(user_id=current_user.id).order_by(MockInterview.started_at.desc()).all()
    
    # Calculate stats
    total_sessions = len(interviews)
    avg_score = int(sum(i.score for i in interviews if i.score) / total_sessions) if total_sessions > 0 else 0
    best_score = max((i.score for i in interviews if i.score), default=0)
    
    # Check for streak (consecutive days with an interview)
    # Simple calculation for now: count interviews in the last 7 days
    from datetime import datetime, timedelta
    recent_interviews = [i for i in interviews if i.started_at > datetime.utcnow() - timedelta(days=7)]
    streak = len(set(i.started_at.date() for i in recent_interviews))
...
def mock_interview():
    ...
    return render_template('user/mock_interview_session.html', hide_nav=True, hide_dock=True)
...
def api_skill_tree():
    ...
    # Define a basic skill tree structure based on level
    # In a real app, this would be more dynamic
    skills = [
        {"id": "base", "name": "Core Professional", "level": 1, "unlocked": True, "x": 0, "y": 0},
        {"id": "resume", "name": "Resume Optimization", "level": 2, "unlocked": xp_profile.level >= 2, "x": -100, "y": 100},
        {"id": "interview", "name": "Interview Mastery", "level": 3, "unlocked": xp_profile.level >= 3, "x": 100, "y": 100},
        {"id": "multimodal", "name": "Sentient Awareness", "level": 5, "unlocked": xp_profile.level >= 5, "x": 0, "y": 200},
    ]
...
@user.route('/api/ats_simulator')
@login_required
@csrf.exempt
def api_ats_simulator():
    latest_resume = UserData.query.filter_by(user_id=current_user.id).order_by(UserData.uploaded_at.desc()).first()
    if not latest_resume:
...
@user.route('/api/resume_builder', methods=['POST'])
@login_required
@csrf.exempt
def api_resume_builder():
    ...
    try:
        filename = f"resume_{current_user.id}_generated_{int(time.time())}.pdf"
        save_path = os.path.join(current_app.config['UPLOAD_FOLDER'], filename)
        generate_resume_pdf(data, save_path)
        ...
        return jsonify({
            'success': True,
            'download_url': url_for('user.uploaded_file', filename=filename, _external=True)
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
@user.route('/skillfit_interview')
@login_required
def skillfit_interview():
    return render_template('user/mock_interview_session.html', hide_nav=True, hide_dock=True, is_skillfit=True)

@user.route('/api/skillfit_dialect_detect', methods=['POST'])
@login_required
def api_skillfit_dialect_detect():
    data = request.get_json()
    audio_transcript = data.get('transcript', '')
    
    # AI logic to detect district/dialect from transcript
    prompt = f"""
    Analyze this Kannada transcript and identify the specific Karnataka district or dialect.
    Transcript: "{audio_transcript}"
    
    Return JSON:
    {{
        "detected_district": "e.g., Dharwad",
        "dialect_tone": "e.g., North Karnataka",
        "suggested_greeting": "Greeting in that specific dialect"
    }}
    """
    result = _call_gemini(prompt, response_mime_type='application/json')
    return jsonify(result or {"detected_district": "General", "dialect_tone": "Formal", "suggested_greeting": "Namaskara"})
