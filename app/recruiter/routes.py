...
def dashboard():
    ...
    if query:
        candidates = recruiter_service.search_talent(query)
    ...
    recruiter_job_ids = [j.id for j in JobPosting.query.filter_by(created_by=current_user.id).all()]
    total_applications = JobApplication.query.filter(JobApplication.job_id.in_(recruiter_job_ids)).count() if recruiter_job_ids else 0
    shortlisted_count = JobApplication.query.filter(
        JobApplication.job_id.in_(recruiter_job_ids),
        JobApplication.status == 'shortlisted'
    ).count() if recruiter_job_ids else 0
...
@recruiter_required
def manage_jobs():
    # Show only jobs created by this recruiter (or all if admin)
...
def create_job():
    ...
    if form.validate_on_submit():
        job = JobPosting(
            ...
            is_ai_round_enabled=form.is_ai_round_enabled.data,
            ai_interviewer_name=form.ai_interviewer_name.data,
            ai_interview_tone=form.ai_interview_tone.data,
            # Associate with organization if profile exists
            organization_id=current_user.recruiter_profile.organization_id if current_user.recruiter_profile else None
        )
        db.session.add(job)
        db.session.commit()
    ...
    return render_template('recruiter/job_form.html', form=form, title="Create New Job")
...
def edit_job(job_id):
    ...
    if form.validate_on_submit():
        ...
        job.is_ai_round_enabled = form.is_ai_round_enabled.data
        job.ai_interviewer_name = form.ai_interviewer_name.data
        job.ai_interview_tone = form.ai_interview_tone.data
        db.session.commit()
    ...
    return render_template('recruiter/job_form.html', form=form, title="Edit Job")
...
@recruiter.route('/jobs/applicants/<int:job_id>')
@recruiter_required
def job_applicants(job_id):
    ...
    return render_template('recruiter/job_applicants.html', job=job, applicant_data=applicant_data)

@recruiter.route('/application/status/<int:app_id>/<status>')
@recruiter_required
def update_application_status(app_id, status):
    ...
    valid_statuses = ['submitted', 'shortlisted', 'ai_interview', 'interviewing', 'rejected', 'hired']
    if status in valid_statuses:
        application.status = status
        
        # Notify Candidate
        msg = f"Update on your application for '{job.title}': {status.replace('_', ' ').capitalize()}"
    ...
    return redirect(url_for('recruiter.job_applicants', job_id=job.id))

@recruiter.route('/candidate/<int:user_id>')
@login_required
def candidate_profile(user_id):
    ...
    insight = recruiter_service.get_candidate_insight(user_id, job_context)
    
    return render_template('recruiter/candidate_profile.html', 
